...
class MACECalculator(Calculator):
    ...
    def _atoms_to_batch(self, atoms):
        self.arrays_keys.update({self.charges_key: "charges", self.magmom_key : "dft_magmom"})
...
class MagneticMACECalculator(Calculator):
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        if self.model_type in ["MACE", "EnergyDipoleMACE"]:
            batch = self._clone_batch(batch_base)
            node_heads = batch["head"][batch["batch"]][:num_real_atoms]
            num_atoms_arange = torch.arange(num_real_atoms)
            node_e0 = self.models[0].magmom_mace.atomic_energies_fn(batch["node_attrs"][:num_real_atoms])[
                num_atoms_arange, node_heads
            ]
        ...
        ret_tensors = self._create_result_tensors(
            self.model_type, self.num_models, num_real_atoms
        )
    ...
    def get_hessian(self, atoms=None):
        ...
        for model in self.models:
            ...
            h = out["hessian"]
            if is_padded:
                h = h[:num_real_atoms, :, :num_real_atoms, :]
    ...
    def get_descriptors(self, atoms=None, invariants_only=True, num_layers=-1):
        ...
        for model in self.models:
            ...
            if is_padded:
                out = self._slice_real_outputs(out, num_real_atoms)
            descriptors.append(out["node_feats"])
