...
class AtomicData(torch_geometric.data.Data):
    ...
    def __init__(
        ...
        magmom_weight: Optional[torch.Tensor] = None,  # [,]
        magforces_weight: Optional[torch.Tensor] = None,  # [,]
    ...
    def from_config(
        ...
        **kwargs,  # pylint: disable=unused-argument
        ...
        magmom_weight = (
            torch.tensor(config.property_weights.get("magmom_weight"), dtype=torch.get_default_dtype())
            if config.property_weights.get("magmom_weight") is not None
            else torch.tensor(1.0, dtype=torch.get_default_dtype())
        )

        magforces_weight = (
            torch.tensor(config.property_weights.get("magforces_weight"), dtype=torch.get_default_dtype())
            if config.property_weights.get("magforces_weight") is not None
            else torch.tensor(1.0, dtype=torch.get_default_dtype())
        )
